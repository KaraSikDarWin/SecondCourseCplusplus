...
bool DataLowX(Elem* est, string vst){//Нод-дата меньше x
    ...
    if (est->data->FB!=vst[0]){
        if ((est->data->FB > vst[0]))  return true; else return false;
    }

    if (est->data->number!=stoi(vst.substr(1,3))) {
        if (est->data->number > stoi(vst.substr(1,3)))
            return true;
        else return false;
    }

    if (est->data->SB[0]!=vst[4]){
        if (est->data->SB[0] > vst[4]) return true; else return false;
    }

    if (est->data->SB[1]!=vst[5]){
        if (est->data->SB[1] > vst[1]) return true;  else return false;
    }
    return 0;
}//
